...
local function setup_pyright()
    ...
    -- 延迟配置：在 Python 文件打开时再初始化 LSP，确保 venv 已激活
    vim.api.nvim_create_autocmd('FileType', {
        pattern = 'python', group = aug,
        callback = function()
            M.activate_venv()
            local python_bin = config.python_path or vim.fn.exepath('python')
            lspconfig.pyright.setup{
                cmd = { python_bin, '-m', 'pyright_langserver', '--stdio' },
                settings = {
                    python = {
                        analysis = {
